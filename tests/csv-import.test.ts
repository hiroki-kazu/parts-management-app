...
describe("history CSV imports", () => {
  it("generates history templates with a part number column", async () => {
    ...
    expect(outboundTemplate.split("\n")[0]).toBe(
      "日付,伝票番号,車両ナンバー,顧客名,品番,部品名,数量",
    );
    expect(outboundTemplate).toContain(
      "2026-09-01,OUT-001,1234,山田自動車,EO-001,エンジンオイル,2",
    );
  ...
  it("re-imports an outbound CSV generated by the app and persists the row", async () => {
    ...
    expect(await getCustomerByVehicleNumber("1234")).toMatchObject({
      name: "山田自動車",
    });
  });

  it("exports every outbound record instead of limiting the template to five rows", async () => {
    const records = Array.from({ length: 6 }, (_, index) => ({
      id: `out-${index + 1}`,
      date: "2026-09-02",
      voucherNumber: `OUT-${index + 1}`,
      customerName: `顧客${index + 1}`,
      vehicleNumber: String(1000 + index),
      partId: "part-1",
      partName: "エンジンオイル",
      quantity: 1,
      createdAt: "2026-09-02T00:00:00.000Z",
    }));
    storage.set("outbound_records", JSON.stringify(records));

    const generatedCSV = await generateOutboundRecordsCSVTemplate();

    expect(parseCSV(generatedCSV)).toHaveLength(7);
    expect(generatedCSV).toContain("OUT-6");
  });

  it("normalizes a formatted vehicle number and imports all outbound fields", async () => {
    const result = await importOutboundRecordsFromCSV(
      "日付,伝票番号,車両ナンバー,顧客名,品番,部品名,数量\n" +
        "2026/09/02,OUT-ALL,12-34,山田自動車,EO-001,エンジンオイル,2\n",
    );
  ...
  it("reports missing required outbound columns before importing", async () => {
    const result = await importOutboundRecordsFromCSV(
      "日付,伝票番号,品番,数量\n2026-09-02,OUT-INVALID,EO-001,2\n",
    );

    expect(result.success).toBe(0);
    expect(result.failed).toBe(1);
    expect(result.errors[0]).toContain("車両ナンバー");
    expect(result.errors[0]).toContain("顧客名");
  });
